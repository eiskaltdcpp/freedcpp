...
void ConnectionManager::on(TimerManagerListener::Second, uint32_t aTick) noexcept {
...
void ConnectionManager::on(TimerManagerListener::Minute, uint32_t aTick) noexcept {
	Lock l(cs);
...
void ConnectionManager::on(UserConnectionListener::Direction, UserConnection* aSource, const string& dir, const string& num) noexcept {
	...
	if(dir == "Upload") {
		// Fine, the other fellow wants to send us data...make sure we really want that...
