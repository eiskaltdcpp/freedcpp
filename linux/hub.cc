...
void Hub::onSendMessage_gui(GtkEntry *entry, gpointer data)
{
	...
	if (text[0] == '/')
	{
		...
		{
			hub->addStatusMessage_gui(string(GUI_PACKAGE " " GUI_VERSION_STRING "." GUI_VERSION_BUILD_STRING "/" VERSIONSTRING ", ") + _("project home: ") + "https://github.com/eiskaltdcpp/freedcpp", Msg::SYSTEM, Sound::NONE);
		}
...
void Hub::connectClient_client(string address, string encoding)
{
	...
	if (address.substr(0, 6) == "adc://" || address.substr(0, 7) == "adcs://")
		encoding = "UTF-8";
	else if (encoding.empty() || encoding == "Global hub default") // latter for 1.0.3 backwards compatibility
